...
The first-stage cleaning workflow is implemented in a Jupyter notebook. The raw Excel workbook is kept unchanged, while the cleaned dataset is exported to Excel after final validation. Quality checks, audit logs, validation tables, and charts are displayed inside the notebook.

The second-stage exploratory analysis starts in a separate notebook under `analysis_code/`. It uses the cleaned dataset to answer business questions about the current customer base.
...
- Phase 2 analysis notebook: `analysis_code/customer_profile_analysis.ipynb`
- The Phase 2 notebook reads the cleaned workbook and answers Question 1: who the store's main customers are by gender, age group, membership tier, device, and payment method.
...
Open the Phase 2 customer profile analysis notebook:

```powershell
jupyter lab analysis_code\customer_profile_analysis.ipynb
```

Run all cells in order after the cleaned dataset exists. The notebook creates the summary table, customer-profile charts, interpretation notes, and final answer for Question 1.
